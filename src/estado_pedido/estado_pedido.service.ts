...
constructor (
  ...
  async create(createEstadoPedidoDto: CreateEstadoPedidoDto,id:number):Promise <EstadoPedido> {
    try{
  ...
  async remove(id: number) {
    const estadoPedido = await this.estPedidoRepository.findOne({where:{id}})
    if(!estadoPedido){
